...
const Transform = {
  ...
  */
  transformSplitNodeSplitNode: (op1, op2, side) => {
    ...
    if (_slate.PathUtils.isAbove(op1.get('path'), op2.get('path'))) {
      const op1Target = op1.get('target');
      const op2Target = op2.get('target');

      if (!(0, _fp.isNil)(op1Target) && !(0, _fp.isNil)(op2Target)) {
        if (op1Target > op2Target || op1Target === op2Target && side === 'right') {
          return _slate.Operation.create({
            object: 'operation',
            type: 'split_node',
            path: _slate.PathUtils.transform(op1.get('path'), op2).get('1'),
