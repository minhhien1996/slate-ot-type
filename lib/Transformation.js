...
exports.OperationTypes = exports.Transform = void 0;
...
const nodeTransformHelper = (op1, op2) => {
  const newPath = _slate.PathUtils.transform(op1.get('path'), op2).get(0);

  if (newPath) {
    return _slate.Operation.create({ ...op1.toJSON(),
      path: newPath
    });
  }

  if (op2.get('type') === 'remove_node' && _slate.PathUtils.isAbove(op2.get('path'), op1.get('path'))) {
    return [];
  }

  if (op1.get('type') === 'insert_node') {
    return op1;
  }
...
const Transform = {
  ...
  transformInsTextInsText: (op1, op2, side) => {
    const pathCompare = _slate.PathUtils.compare(op1.get('path'), op2.get('path'));
  ...
  transformInsTextRemoveText: (op1, op2, side) => {
    ...
    if (pathCompare === 0) {
      ...
      const op2EndPoint = op2StartPoint + op2Len; // if op1 happens completely before op2
  ...
  transformInsTextAddMark: (op1, op2, side) => {
    ...
    if (pathCompare === 0) {
      ...
      if (op1.get('offset') >= op2.get('offset') + op2.get('length')) {
        return op1;
      ...
      newMarks = newMarks.add(_slate.Mark.create({
        type: mark.type,
        data: {}
      }));
      return _slate.Operation.create({
  ...
  transformInsTextRemoveMark: (op1, op2, side) => {
    ...
    if (pathCompare === 0) {
      ...
      const mark = op2.get('mark');
      const newMarks = op1.get('marks').filterNot(e => e.get('type') === mark.get('type'));
      return _slate.Operation.create({
        object: 'operation',
        type: 'insert_text',
        path: op1.get('path'),
   ...
   */
  transformAddMarkAddMark: (op1, op2, side) => op1,

  /**
   * [add_mark, add_mark] transformation.
   */
  transformRemoveMarkRemoveMark: (op1, op2, side) => op1,

  /**
   * [add_mark, remove_mark] transformation.
   */
  transformAddMarkRemoveMark: (op1, op2, side) => op1,

  /**
   * [remove_mark, add_mark] transformation.
   */
  transformRemoveMarkAddMark: (op1, op2, side) => op1,
  ...
  transformRemoveTextInsertText: (op1, op2, side) => {
    ...
    if (pathCompare === 0) {
      ...
      const op1StartPoint = op1.get('offset');
      const op1EndPoint = op1StartPoint + op1Len;
      const op2StartPoint = op2.get('offset'); // if op1 happens completely before op2
      // no transforming is needed
  ...
  transformInsertNodeInsertNode: (op1, op2, side) => {
    const pathCompare = _slate.PathUtils.compare(op1.get('path'), op2.get('path')); // handle ties
  ...
  /**
   * [split_node, merge_node] transformation.
   */
  transformSplitNodeMergeNode: (op1, op2, side) => op1,
  ...
  */
  transformSetNodeSplitNode: (op1, op2, side) => {
    if (_slate.PathUtils.compare(op1.get('path'), op2.get('path'))) {
      _slate.Operation.create({
        object: 'operation',
        type: 'set_node',
        path: _slate.PathUtils.transform(op1.get('path'), op2).get('0'),
  ...
  */
  transformSplitNodeSplitNode: (op1, op2, side) => {
    ...
    if (pathCompare === 0) {
      ...
      if (op1StartPoint > op2StartPoint || op1StartPoint === op2StartPoint && side === 'right') {
        return _slate.Operation.create({
          object: 'operation',
          type: 'split_node',
          path: _slate.PathUtils.transform(op1.get('path'), op2).get('1'),
    ...
    const op1AboveOp2 = _slate.PathUtils.isAbove(op1.get('path'), op2.get('path'));

    if (op1AboveOp2) {
      const op1Target = op1.get('target');
      const op2Position = op2.get('position');

      if (op1Target > op2Position || op1Target === op2Position && side === 'right') {
        return _slate.Operation.create({
          object: 'operation',
          type: 'split_node',
          path: _slate.PathUtils.transform(op1.get('path'), op2).get('0'),
    ...
    const op2AboveOp1 = _slate.PathUtils.isAbove(op2.get('path'), op1.get('path'));

    if (op2AboveOp1) {
      const op1Position = op1.get('position');
      const op2Target = op2.get('target');

      if (op1Position > op2Target || op1Position === op2Target && side === 'right') {
        return _slate.Operation.create({
          object: 'operation',
          type: 'split_node',
          path: _slate.PathUtils.transform(op1.get('path'), op2).get('0'),
